...
/*
    ...
    if( j < 4 )
    {
        ...
        double currentTolerance = std::pow( 10.0, (-10.0 + static_cast< double >( k ) ) );       // modify these!!!!!!!!!!!!!!!!!

        // Create integrator settings
        return std::make_shared< RungeKuttaVariableStepSizeSettings< > >(
                    simulationStartEpoch, 0.02, currentCoefficientSet,
    ...
    {
        // Extract tolerance for current run
        double currentTolerance = std::pow( 10.0, (-10.0 + static_cast< double >( k ) ) );       // modify these!!!!!!!!!!!!!!!!!

        // Create integrator settings
        return std::make_shared< BulirschStoerIntegratorSettings< > >(
                    simulationStartEpoch, 0.02, bulirsch_stoer_sequence, 10,
    ...
    {
        // Extract integrator type and tolerance for current run
        RungeKuttaCoefficients::CoefficientSets currentCoefficientSet = multiStageTypes.at( j-6 );
        Eigen::Vector4d steplist(0.5, 0.1, 0.05, 0.02);
        double timeStep = steplist(k);
    ...
    else if( j == 11 )
    {
        ...
        // Create integrator settings
        return std::make_shared< BulirschStoerIntegratorSettings< > >(
                    simulationStartEpoch, timeStep, bulirsch_stoer_sequence, 10,
                    timeStep, timeStep, 1, 1);}

*/

    //Uncomment this for Question 3: dont forget to set number of j = 1
/*
    if( j == 0 )
    {
        double timeStep = 0.5;
        ...
        return std::make_shared< RungeKuttaVariableStepSizeSettings< > >(
                    ...
                    timeStep, timeStep, std::numeric_limits< double >::epsilon( ), std::numeric_limits< double >::infinity( ) );
    }
    */
...
// Generate most accurate benchmark solution :
// Step size analysis
{
    double run = 101; //101;
    double interval = 0.01;//0.01 ;
    ...
    double firstBenchmarkStepSize = 0.0005; //interval*(run-1); //interval*(run-1)*0.5;
    ...
    benchmarkIntegratorSettings = std::make_shared< RungeKuttaVariableStepSizeSettings< > >(
                simulationStartEpoch, firstBenchmarkStepSize, RungeKuttaCoefficients::rungeKuttaFehlberg45,
    ...
    for (int i = 1; i < run; i++)
    {
     double benchstep = interval+interval*(i-1)*0.5;

    // Create integrator settings for 2nd run
    double secondBenchmarkStepSize = benchstep;
    benchmarkIntegratorSettings = std::make_shared< RungeKuttaVariableStepSizeSettings< > >(
                simulationStartEpoch, secondBenchmarkStepSize, RungeKuttaCoefficients::rungeKuttaFehlberg45,
    ...
    input_output::writeDataMapToTextFile( secondBenchmarkStates, "benchmark_step" + std::to_string(i) + ".dat", outputPath );
    input_output::writeDataMapToTextFile( secondBenchmarkDependent, "benchmarkDependent_step"+ std::to_string(i) + ".dat", outputPath );
    ...
    input_output::writeDataMapToTextFile( statesDifference,
                                          "benchmarkStateDifference_step"+ std::to_string(i) + ".dat", outputPath );
    input_output::writeDataMapToTextFile( dependentVariablesDifference,
                                          "benchmarkDependentDifference_step" + std::to_string(i) + ".dat", outputPath );
...
std::vector< std::shared_ptr< OneDimensionalInterpolator< double, Eigen::VectorXd > > > comparewithUSM(
       const double simulationStartEpoch, const double specificImpulse, const simulation_setup::NamedBodyMap& bodyMap,
       const std::shared_ptr< MultiTypePropagatorSettings< double > > benchmarkPropagatorSettings,
        const std::shared_ptr< MultiTypePropagatorSettings< double > > benchmarkPropagatorSettingssecond,
       std::vector< double > thrustParameters, std::string outputPath )

{
    std::vector< std::shared_ptr< OneDimensionalInterpolator< double, Eigen::VectorXd > > > interpolators;
    // Put the benchmark data in a separate directory
    outputPath.append("/bench_prop/");
    // Create integrator settings for 1st run
    ...
    // Write the state maps of both benchmarks to files
    input_output::writeDataMapToTextFile( secondBenchmarkStates, "benchmark_propanal.dat", outputPath );
    input_output::writeDataMapToTextFile( secondBenchmarkStates, "benchmarkDependent_propanal.dat", outputPath );
    ...
    // Write the difference in state and dependent variables between the two benchmarks to files
    input_output::writeDataMapToTextFile( statesDifference,
                                          "benchmarkStateDifference_propanal.dat", outputPath );
    input_output::writeDataMapToTextFile( dependentVariablesDifference,
                                          "benchmarkDependentDifference_propanal.dat", outputPath );
...
int main( )
{
    ...
    double maximumDuration = 600.0;  //change this ? - and make sure it produces "good" resutls for bench marking
    ...
    if( generateAndCompareToBenchmark )
    {
        ...
        // Define full propagation settings for the second case (propagation analysis)
        ...
        benchmarkInterpolators = comparewithUSM(simulationStartEpoch, constantSpecificImpulse, bodyMap, benchmarkPropagatorSettings,
                                                benchmarkPropagatorSettingssecond, thrustParameters, outputPath );
    ...
    unsigned int numberOfIntegrators = 1;
